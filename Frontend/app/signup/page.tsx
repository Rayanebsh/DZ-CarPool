        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 space-y-6">
            ...
            <div className="flex gap-2 bg-gray-100 p-1 rounded-lg">
              ...
              <Link href="/login" className="flex-1 py-2.5 text-sm font-medium rounded-md text-center text-gray-600 hover:text-gray-900 transition-colors">
                {t("login")}
              </Link>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input id="email" type="email" placeholder={t("emailPlaceholder")} value={formData.email} onChange={handleInputChange} required />
              </div>

              <div className="space-y-1.5">
                <Label htmlFor="phoneNumber">{t("phoneNumber")}</Label>
                <Input id="phoneNumber" type="tel" placeholder={t("phonePlaceholder")} value={formData.phoneNumber} onChange={handleInputChange} required />
              </div>

              <div className="space-y-1.5">
                <Label htmlFor="password">{t("password")}</Label>
                <Input id="password" type="password" placeholder={t("passwordPlaceholder")} value={formData.password} onChange={handleInputChange} required />
              </div>

              <div className="space-y-1.5">
                <Label htmlFor="passwordConfirm">{t("confirmPassword")}</Label>
                <Input id="passwordConfirm" type="password" placeholder={t("confirmPasswordPlaceholder")} value={formData.passwordConfirm} onChange={handleInputChange} required />
              </div>

              <div className="space-y-1.5">
                <Label>{t("uploadIdPhoto")} <span className="text-gray-400 font-normal">({t("optional")})</span></Label>
                <div
                ...
                >
                  ...
                  <input ref={fileInputRef} type="file" accept="image/*" onChange={handleIdPhotoUpload} className="hidden" />
                </div>
              </div>

              <Button type="submit" disabled={loading} className="w-full h-12 bg-[#FF5722] hover:bg-[#E64A19] text-white font-medium">
            ...
            <p className="text-center text-xs text-gray-500 leading-relaxed">
              {t("agreeTerms")} <Link href="/terms" className="text-[#FF5722] hover:underline">{t("termsOfService")}</Link> {t("and")} <Link href="/privacy" className="text-[#FF5722] hover:underline">{t("privacyPolicy")}</Link>.
            </p>
          </div>
        </div>
      </div>
