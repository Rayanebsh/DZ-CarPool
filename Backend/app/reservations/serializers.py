...
class ReservationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Reservation
        fields = [
            'id', 'trajet', 'trajet_detail', 'passager', 'passager_detail',
            'nbr_places', 'status', 'price_per_seat', 'total_price',
            'created_at', 'approved_at', 'cancelled_at', 'rejection_reason',
            'cancellation_reason', 'can_rate'
        ]
        read_only_fields = [
            'id', 'passager', 'created_at', 'approved_at', 'cancelled_at',
    ...
    def get_can_rate(self, obj):
        ...
        is_completed = (
            ...
            obj.trajet.date < timezone.now().date()
        )
...
class RatingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Rating
        fields = [
            'id', 'reservation', 'reservation_detail', 'rater', 'rater_detail',
            'rated', 'rated_detail', 'note', 'comment', 'ponctualite',
            'convivialite', 'conduite', 'created_at'
        ]
    ...
    def validate(self, attrs):
        ...
        if Rating.objects.filter(reservation=reservation, rater=request.user).exists():
            raise serializers.ValidationError(
                "Vous avez déjà noté cette réservation"
            )
