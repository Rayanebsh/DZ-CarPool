...
from .serializers import (
    TrajetListSerializer, TrajetDetailSerializer, TrajetCreateSerializer,
    TrajetUpdateSerializer, TrajetSearchSerializer, FuelPriceSerializer
)
from .permissions import (
    IsDriverOrReadOnly, CanModifyTrajet, CanCancelTrajet, 
...
class TrajetViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == 'create':
            ...
            permission_classes = [permissions.IsAuthenticated, IsDriverOrReadOnly, CanModifyTrajet]
    ...
    @action(detail=True, methods=['post'], permission_classes=[permissions.IsAuthenticated, CanCancelTrajet])
    def cancel(self, request, pk=None):
        ...
        for reservation in trajet.reservations.filter(status__in=['PENDING', 'CONFIRMED']):
    ...
    @action(detail=True, methods=['get'], permission_classes=[permissions.IsAuthenticated, CanViewTrajetReservations])
    ...
    def statistics(self, request, pk=None):
        ...
        stats = {
            ...
            'places_reserved': sum(r.nbr_places for r in reservations.filter(status='CONFIRMED')),
            'places_disponibles': trajet.places_disponibles,
            'revenue_estimate': sum(r.total_price for r in reservations.filter(status='CONFIRMED')),
        }
