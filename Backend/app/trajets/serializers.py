...
class TrajetListSerializer(serializers.ModelSerializer):
    ...
    conducteur_name = serializers.CharField(source='conducteur.full_name', read_only=True)
    ...
    conducteur_picture = serializers.ImageField(source='conducteur.profile_picture', read_only=True)
    
    class Meta:
        model = Trajet
        fields = [
            'id', 'conducteur', 'conducteur_name', 'conducteur_rating', 'conducteur_picture',
            'ville_depart', 'ville_arrivee', 'date', 'heure_depart',
            'nbr_places', 'places_disponibles', 'price', 'distance',
            'is_confort', 'pause_required', 'status', 'created_at'
        ]
...
class TrajetDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Trajet
        fields = [
            'id', 'conducteur', 'ville_depart', 'ville_arrivee',
            'adresse_depart', 'adresse_arrivee', 'date', 'heure_depart',
            'nbr_places', 'places_disponibles', 'price', 'price_platform',
            'price_driver', 'suggested_price', 'distance', 'is_confort',
            'pause_required', 'status', 'description', 'luggage_allowed',
            'etapes', 'total_reservations', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'conducteur', 'price_platform', 'price_driver',
            'places_disponibles', 'pause_required', 'created_at', 'updated_at'
        ]
...
class TrajetCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Trajet
        fields = [
            'ville_depart', 'ville_arrivee', 'adresse_depart', 'adresse_arrivee',
            'date', 'heure_depart', 'nbr_places', 'price', 'distance',
            'is_confort', 'description', 'luggage_allowed', 'etapes'
        ]
    ...
    def validate_distance(self, value):
        """Valide la distance"""
        if value < 1:
            raise serializers.ValidationError("La distance doit être supérieure à 0 km.")
        return value
...
class TrajetUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'adresse_depart', 'adresse_arrivee', 'date', 'heure_depart',
            'nbr_places', 'price', 'is_confort', 'description',
            'luggage_allowed', 'status'
        ]
